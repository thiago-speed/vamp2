...
# Inicialização de fontes melhoradas
pygame.font.init()
try:
    # Tentar usar fontes do sistema mais bonitas
    ...
    # Tentar carregar fontes específicas se disponíveis
    try:
...
def desenhar_texto(tela, texto, pos, cor, tamanho, centralizado=False, sombra=True):
    """Desenha texto com sombra e melhor estilização"""
...
def desenhar_barra_vida(tela, pos, hp_atual, hp_max, largura=40, altura=6):
    """Desenha barra de vida melhorada"""
    if hp_max <= 0:
        return
    ...
    # Desenhar fundo da barra (mais escuro)
...
# Sistema de partículas para cenário
...
class CenarioEspacial:
    ...
    def gerar_fundo_inicial(self):
        """Gera partículas iniciais do cenário"""
    ...
    def atualizar(self):
        """Atualiza partículas do cenário"""
        ...
        # Spawnar novas partículas ocasionalmente
        self.tempo_spawn += 1
        if self.tempo_spawn > 120:  # A cada 2 segundos
            self.tempo_spawn = 0
            if len(self.particulas) < 250:  # Limite máximo
                x = random.randint(0, LARGURA_MAPA)
                y = random.randint(0, ALTURA_MAPA)
    ...
    def desenhar(self, tela, camera):
        ...
        # Desenhar gradient de fundo (simulado com retângulos)
        for i in range(0, ALTURA_TELA, 20):
            intensidade = int(5 + (i / ALTURA_TELA) * 15)  # Gradiente sutil
        ...
        # Adicionar alguns elementos fixos decorativos
        self.desenhar_elementos_decorativos(tela, camera)
    
    def desenhar_elementos_decorativos(self, tela, camera):
        """Desenha elementos decorativos fixos"""
        # Algumas "galáxias" distantes
        posicoes_galaxias = [
        ...
        for pos in posicoes_galaxias:
            if esta_na_tela(pos, camera, 100):
                ...
                # Desenhar galáxia espiral simulada
                for i in range(3):
                    raio = 15 + i * 8
                    intensidade = 30 - i * 8
